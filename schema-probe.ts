    ...
    if (error) {
      ...
      if (matchCol) {
        ...
        if (matchNotNull) {
          ...
          console.log(`Table ${table} hit foreign key constraint, schema must be:`, Object.keys(dummy));
...
async function run() {
  const tables = ['product_images', 'product_colors', 'product_sizes', 'product_specifications', 'product_details'];
